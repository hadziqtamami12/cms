...
export const getActiveThemeConfig = async () => {
  const cached = await getCache('sys:active_theme');
...
export const updateActiveThemeConfig = async (partialConfig) => {
  currentThemeConfig = {
    ...
    ...partialConfig,
    seo: {
      ...currentThemeConfig.seo,
      ...(partialConfig.seo || {})
    }
  };
  await setCache('sys:active_theme', currentThemeConfig, 300);
  return currentThemeConfig;
};

export const switchThemeVariant = async ({ industry, themeId, bottomNavStyle }) => {
  if (industry) currentThemeConfig.industry = industry;
  if (themeId) currentThemeConfig.themeId = themeId;
  if (bottomNavStyle) currentThemeConfig.bottomNavStyle = bottomNavStyle;
  await setCache('sys:active_theme', currentThemeConfig, 300);
  return currentThemeConfig;
};
